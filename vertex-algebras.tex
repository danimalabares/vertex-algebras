...
\noindent
which clearly surjective since we can put
$$
\begin{pmatrix}
A&0\\ 
B&C
...
and we can form its pushout by the following
Exercise \ref{exercise-pushout}.
...
Explicitly,
$$
K=\frac{K \oplus B}{\left<(t(a),0)-(0,i(a))|a \in A\right>}.
$$
...
(as associative and as Lie algebras),
and
$$
\xymatrix{
...
\mathfrak{a}_1&=\text{End}_d(X)\\
\mathfrak{a}_2&=\text{End}_c(X)\\
\mathfrak{a}_0&=\text{End}_f(X)\\
X& \text{ our Tate vector space}\\
T \text{trace}.
\end{align*}
...
$$
\underbrace{
f^{-1}g^{-1}(0)}_{\subset f^{-1}(U)}
\xrightarrow{f}\underbrace{g^{-1}(0)
}_{\subsetU}\xrightarrow{g}0
$$
and 
$$
