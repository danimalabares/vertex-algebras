...
We would like to interpret such relations at the level of $h(z)$.

\begin{align*}
[h(z),h(z)]&=h(z)h(z)-h(z)h(z)\\
&=\sum_{p}\sum_{m+n=p}h_mh_nz^{-(m+n)-2}
-\sum_{m+n=p}h_nh_mz^{-(m+n)-2}\\
&=\sum_p z^{-p-2}\underbrace{\left(\sum_{m+n=p}[h_m,h_n]\right)}
_{\text{infinite?}}
\end{align*}
...
$h_n$ {\bf for $n \neq 0$}.
In other words, it commutes with $\mathfrak{a}$ as defined above.
...
$(\cdot,\cdot)$ a symmetric invariant bilinear form.
We have the affine Lie algebra 
$\hat{\mathfrak{g}}=\mathfrak{g}[t^{\pm 1}\oplus \mathbb{C}K$.
We defined a smooth $\hat{\mathfrak{g}}$-module
and level $k$ (which means that $K \mapsto  k \text{Id}$
$\hat{\mathfrak{g}}$-modules,
and the vertex algebra $V^k(\mathfrak{g})$.
Again recall that $V^k(\mathfrak{g})$-modules
are in correspondence with smooth level $k$ $\hat{\mathfrak{g}}$-modules.

Consider $U$ an associative algebra over $\mathbb{C}$ with
$U_0 \supset U_1 \supset U_2 \supset\ldots$ a decreasing
filtration by subspaces $U_p$, such that
\begin{enumerate}
\item
\label{item-separating}
(Separating.) $\bigcap_{p}U_p=0$.
[The idea here is that the $U_p$ may be thought of
as a topological basis, and this condition means
the algebra, as a topological algebra, separates points:
there's an open set containing zero but not any given point.]

\item 
\label{item-l}
For any $u \in U$ and $p\geq 0$, there exists $\ell$
wuch that  $U_\ell \cdot u \subset U U_p$.
(think: for any $p$, no matter how big,
there exists $\ell$, maybe much bigger, such that  …)
[The idea here is that we want to turn the  $U_p$ into ideals,
just by multiplying by $U$ --- indeed, $U U_p$ is an ideal.]

\item (Exhaustive.) $U_0= U$? Maybe not necessary.
\label{item-exhaustive}
\end{enumerate}

\noindent
We say a left $U$-module $M$ is {\it smooth} if
for all $x \in M$ there exists $p$ such that $U_p\cdot x=0$.

\medskip\noindent
Given $U$ and $\{U_p\}$ as above, we can form the {\it completion} $U^c$
as a categorical limit of the diagram

$$
\xymatrix{
U/U U_0&U/U U/1 \ar[l]& U/ U U_2 \ar[l]& \cdots \ar[l].
}
$$
Can think of $U^c$ as
$$
...
=\left\{(u_0,u_1,u_2,\ldots):\substack{u_i \in U/U U_i \\ 
\text{and }\overline{u_{i+1}}=u_i}\right\}.
$$
One can then set $s_i=u_i-u_{i-1}$, then
$$
(u_0,u_1,u_2,\ldots)=(s_0,s_0+s_1,s_0+s_1+s_2,\ldots)
\text{``=''}\sum_is_i
$$
and we recover the first description.

Clearly the action of $U$ on a smooth $U$-module $M$ 
extends to a well-defined action of $U^c$.
Now let $a(w)=\sum_{n \in \mathbb{Z}}a_{(n)}w^{-n-1}
\in U^c[[w^{\pm}]]$,
we call $a(w)$ a {\it continuous distribution} if
for every $p \geq 0$ there exists $N \geq 0$ such that
$a_{(n)}\in U U_p$ for all $n \geq N$.
We denote the vector space of continuous distrinutions by
$U^c[[w]]^{cont}$.

Then it's clear that
\begin{lemma}
\label{lemma-continuous-distribution-is-quantum}
If $M$ is a smooth $U$-module
(denote action of $a_{(n)}$ on $M$ by $a^M_{(n)}\in \text{End}(M)$,
...
the $n$-th product
 $$
a(w)_{(n)}b(w)
=\text{Res}_z(a(z)b(w)i_{z,w}-b(w)a(z)i_{w,z})(z-w)^n
$$
is well defined and lies in $U^c[[w^{\pm 1}]]^{cont}$.
\end{proposition}

\begin{proof}
Analogous to Exercise \ref{exercise-most-important}.
\end{proof}
...
We say the $a(w),b(w)$ is
 \begin{enumerate}
\item {\it weakly local} if there exists  $N$ such that
$a(w)_{(n)}b(w)=0$ for all $n \geq N$.
\item {\it local} if there exists $N$ such that
$(z-w)^N[a(z),b(w)]=0$.
\end{enumerate}
\end{definition}
...
Let $\mathcal{F}\subset U^c[[w^{\pm}]]^{cont}$ be a subset such that
all pairs of elements are local.
Let $\overline{\mathcal{F}}\subset U^c[[w^{\pm 1}]]^{cont}$ be the
closure of $\mathcal{F}$.
 \begin{align*}
\mathcal{F}^0&=\{1_{U^c}\},\qquad 
\mathcal{F}^k=\left\{a(w)_{(n)}b(w):\substack{a \in \mathcal{F}, \\
b \in \mathcal{F}^{k-1}, \\
n \in \mathbb{Z}}\right\}.
\end{align*}

\noindent
The analogue of Dong's lemma \ref{lemma-dong} guarantees all
...
The vector space $\overline{\mathcal{F}}$, equipped with
$|0\rangle=1_{U^c}$,
$Y(a(w),x)=\sum_{n \in \mathbb{Z}}a(w)_{(n)}x^{-n-1}$,
$Ta(w)=\partial_wa(w)=a(w)_{(-2)}1_{U^c}$ is a vertex algebra.
\end{theorem}
...
Let's assign
$$
Y^M(a(w),x)=a^M(x)=\sum_{n \in \mathbb{Z}}a^M_{(n)}x^{-m-1}.
$$

\begin{proposition}
\label{proposition-smooth-f-module}
$(M,Y^M)$ is an $\overline{\mathcal{F}}$-module.
\end{proposition}
...
$$
because $\overline{\mathcal{F}}$ is a vertex algebra.

So 

\begin{align*}
Y^M(a(w)_{(n)}b(w),x)&=a^M(x)_{(n)}b^M(x)\qquad 
\text{by definition}\\
...
\noindent
[It looks as if this construction was a slight generalization
of our earlier work for vertex algebras over vector spaces
to vertex algebras over associative algebras;
now we have to blend in this topological algebra notions - 
completion, etc.]

\medskip\noindent
Now  let's look at the case of $\hat{\mathfrak{g}}$-modules and
$V^k(\mathfrak{g})$.
We have $\hat{\mathfrak{g}}=\mathfrak{g}[t^{\pm 1}] \oplus \mathbb{C}K$.
Let
$$
...
$\hat{\mathfrak{g}}$-modules.
Now consider $U_p=\text{Im}(\mathfrak{g}t^{\geq p})\subset U$.
I claim $U$ with $U_p$ so defined satisfies the conditions
(\ref{item-separating}) and (\ref{item-l}).
Indeed, (\ref{item-separating}) is given by PBW theorem
since every monomial has a highest degree term.
As for (\ref{item-l}), e.g., for $u=at^{-5}$, $p=10$, if I take
$\ell=15$, then $u_\ell \ni bt^{15}$,
$[bt^{15},at^{-5}]=[b,a]t^{10}$.

Now, in $U^c[[w^{\pm 1}]]^{cont}$ we have, for each
$a \in \mathfrak{g}$, the continuous distribution
$$
a(w)=\sum_{n \in \mathbb{Z}}at^n w^{-n-1}.
$$
By the commutation relation
$[at^m,bt^n]=[a,b]t^{m+n}+m\delta_{m,-n}(a,b)k 1$
we have $(z-w)^2[a(z),b(w)]=0$
so these are mutually local,
($[a(z),b(w)]=[a,b](w)\delta(z,w)+(a,b)k 1 \partial_w \delta(z,w)$.)

So we get a vertex algebra $\overline{\mathcal{F}}$ 
from $\mathcal{F}=\{a(w):a \in \mathfrak{g}\}$
and any smooth  $\hat{\mathfrak{g}}$-module of level $k$ gives
an $\overline{\mathcal{F}}$-module.

\medskip\noindent
In particular 
$V^k(\mathfrak{g})
=U(\hat{\mathfrak{g}})\otimes_{U(\hat{\mathfrak{g}}_+)}\mathbb{C}K$ 
(which happens to be a vertex algebra),
being a smooth level $k$ $\hat{\mathfrak{g}}$-module is an
$\overline{\mathcal{F}}$-module.

In $M=V^k(\mathfrak{g})$, for $a \in \mathfrak{g}$,
$a(w)$ (from $\overline{\mathcal{F}}$) acts via 
$Y^M(a(w),x)=a(x)=T(a_{(-1)}|0\rangle,x)$.

$$
\xymatrix{
a(w)\ar@{|->}[r]&Y(a(w),z)\\
\hat{\mathcal{F}}\ar[r]\ar[rd]& \{Y(a,z):a \in V^k(\mathfrak{g})\}
\ar[d]
& a(z)\ar@{|->}[d]\\
...
$$
\xymatrix{
a(w)_{(n)}b(w)\ar@{|->}[r]&Y(a(w),z)_{(n)}Y(b(w),z)\\
\hat{\mathcal{F}}\ar[r]\ar[rd]_\phi& \{Y(a,z):a \in V^k(\mathfrak{g})\}
\ar[d]
& a(z)_{(n)}b(w)\ar@{|->}[d]\\
&V^k(\mathfrak{g})&a_{(n)}b
}
$$
So $\phi$ is a homomorphism of vertex algebras
\begin{align*}
...
\noindent
Clearly $\phi$ is surjective. Want to show $\Ker(\phi)=0$.

\medskip\noindent
We can characterize $V^k(\mathfrak{g})$ as the universal
highest weight level $k$ $\hat{\mathfrak{g}}$-module,
...
\noindent
then there exists a unique $\hat{\mathfrak{g}}$-module map
\begin{align*}
...
I claim 
\begin{enumerate}
\item $\overline{\mathcal{F}}$ is a $\hat{\mathfrak{g}}$-module,
...
Then
\begin{align*}
[\rho(at^m),\rho(bt^n)]&=[a(w)_{(m)},b(w)_{(n)}\\
&=\sum_{j \geq 0}\binom{m}{j}(a(w)_{(j)}b(w))_{(m+n-j)}\\
&=(*).
\end{align*}
...
Then
\begin{align*}
(*)&=[a,b](w)_{(m+n)}+mk(a,b)1_{U^c}(w)_{(m+n-1)}\\
&=\rho([a,b]t^{m+n}+m\delta_{m,-n}(a,b)K).
\end{align*}

\noindent
So $\rho$ is a representation of
$\hat{\mathfrak{g}}$ on $\overline{\mathcal{F}}$.

Obviously $\rho$ is level $k$
and obviously $x=1_{U^c}(w) \in \overline{\mathcal{F}}$
is highest weight.
$a(w)_{(j)}1(w)=0$ for all $j\geq 0$.

So we have maps
$$
\overline{\mathcal{F}}\underbrace{\xrightarrow{\phi}}
_{\substack{\text{VA homo, hence} \\ \hat{\mathfrak{g}}\text{-mod map}}}
V^k(\mathfrak{g})\underbrace{\xrightarrow{\psi}}_{
\hat{\mathfrak{g}}\text{-mod map}}
\overline{\mathcal{F}}.
$$
Can check $\psi \circ \phi=\text{id}$.
So $\overline{\mathcal{F}}\simeq V^k(\mathfrak{g})$
as vertex algebras.
\end{enumerate}
\end{proof}


\section{Characters of vertex algebras and their modules}
\label{section-characters-of-vertex-algebras}

\noindent
{\bf Disclaimer!} There may be several computational mistakes in this section.

\medskip\noindent
Prelude. Let $q=e^{2\pi i\tau}$, $\tau \in \mathbb{C}$,
$\text{Im}(\tau)>0$ (so  $|q|<1$).
Consider
\begin{align*}
...
\noindent
Then $\sum_{d|n}d^{11}\equiv a_n\text{ mod }691$.
...
by integer eigenvalues with finite dimensional eigenspaces.
Write $V=\bigoplus_{n \in \mathbb{Z}}V_n$,
where $V_n=\{v \in V:L_0v=nv\}$
and write $\chi_V = \sum_{n \in \mathbb{Z}}\dim(V_n)q^n$.

\begin{example}
\label{example-heisenberg-character}
Recall that $V=H=\mathbb{C}[x_1,x_2,\ldots]$
with conformal vector $L=\frac{1}{2}\,:\!hh\!:\,$.
We saw that $L_01=0$, $[L_0,h_n]=nh_n$.So
$$
L_0(h_nv)=h_n(L_0v)-nh_nv.
$$
This means: take a vector with eigenvalue $\Delta$,
then apply again, you'll get an eigenvector with eigenvalue
$\Delta-n$. In symbols: $v \in V_\Delta$, then
$L_0v \in V_{\Delta-n}$.

So $\Delta(x_1^{m_1}x_2^{m_2}\ldots x_k^{m_k}1)=\sum i m_i$
and 
\begin{align*}
...
If $\mathfrak{h}$ is an $n$-dimensional vector space
$(\cdot,\cdot)$ a nondegenerate symmetric bilinear form,
$\mathfrak{h}$ abelian Lie algebra, $\hat{\mathfrak{h}}$ 
affine Lie algebra. Then $H=H(\mathfrak{h})$ 
is the {\it rank $n$ Heisenberg algebra}.
Given a basis $\{a^1,\ldots,a^n\}$ of $\mathfrak{h}$ 
...
\begin{example}
\label{example-lattice-vertex-algebra-character}
Let $Q \subset \mathbb{R}^n$ be an even integral lattice.
Consider 
$\mathfrak{h}=\mathbb{R}^n \otimes_{\mathbb{R}} \mathbb{C}=\mathbb{C}^n$
with $(\cdot,\cdot)$ inherited.
We have $V=V_Q=\bigoplus_{\alpha \in Q}H^\alpha(\mathfrak{h})$.

As part of our construction of $V_Q$ we computed that
$$
L_0|\alpha\rangle=\frac{(\alpha,\alpha)}{2}|\alpha\rangle.
$$
Hence $|\alpha\rangle\in V_{\frac{(\alpha,\alpha)}{2}}$.
(Also $[L_0,h_n]=-nh_n$ 
for all $h \in \mathfrak{h}$ and all $n \in \mathbb{Z}$.)

Also $h_{-n_1}^{i_1}h_{-n_2}^{i_2}\ldots h_{-n_s}^{i_s}|\alpha\rangle$
lies in $V_{\Delta}$, $\Delta=\frac{(\alpha,\alpha)}{2}
+\sum_{j=1}^sn_j$. (Just as above: we apply the operator again
and obtain another eigenvector, this time with eigenvalue given by that sum.)
Therefore
$$
\chi_{V_Q}=\sum_{\alpha \in Q}q^{\frac{(\alpha,\alpha)}{2}}
\prod_{m=1}^\infty \frac{1}{(1-q^m)^n}.
$$
This makes sense, since (for now) $Q$ is assumed to be positive-definite.

In fact
$$
\chi_{V_Q}=\prod_{m=1}^\infty\frac{1}{(1-q^m)^n}\cdot
\underbrace{\sum_{\alpha \in Q}q^{\frac{(\alpha,\alpha)}{2}}}_{
\substack{\text{theta} \\ \text{function}}}.
$$
The series on RHS is called the {\it theta function} 
of the lattice $Q$, denoted $\theta_Q(\tau)$.
(Think of it as a function that knows how many vectors of each
norm the lattice has.)
More precisely, for $q=e^{2\pi i \tau}$,
$$
\theta_Q(\tau)=\sum_{\alpha \in Q}q^{\frac{(\alpha,\alpha)}{2}}
=\sum_{\alpha \in Q}e^{\pi i (\alpha,\alpha)\tau}.
$$

As detour, notice that if $Q$ is integral, but not even,
then some $\frac{(\alpha,\alpha)}{2} \not \in \mathbb{Z}$,
but are in $\frac{1}{2}\mathbb{Z}$.
As an example of this, $Q=\mathbb{Z} \subset \mathbb{R}$ is
an odd lattice,
$$
\theta_Q(\tau)=
\sum_{n \in \mathbb{Z}}q^{\frac{n^2}{2}}
=1+2q^{\frac{1}{2}}+2q^2+2q^{\frac{a}{2}}+\ldots.
$$
\end{example}

\begin{remark}
\label{remark-theta-function-converges}
Note that for any positive definite $Q$ (even or odd)
the series $\theta_Q(\tau)$ converges for all
$\text{Im}(\tau)>0$.
(Easy application of some test or other.)

If $Q$ is even, then only integer powers of $q$ 
appear, and we have
$$
\theta_Q(\tau+1)=\theta_Q(\tau)\qquad \forall \tau\in \mathbb{H}
$$
where $\mathbb{H}=\{\tau \in \mathbb{C}:\text{Im}(\tau)>0\}$.

Can then also think of $\theta_Q$ as a holomorphic
function (of $q$) on the disk
$D=\{q:|q|<1\}$.

If $Q$ is odd, then $\theta_Q(\tau+2)=\theta_Q(\tau)$.
\end{remark}
...
\begin{proposition}
\label{proposition-theta-function-for-self-dual-lattice}
...
\begin{proof}
This is also an application of Poisson summation.
\end{proof}
...
$$
\theta_Q\left(-\frac{1}{\tau}\right)
=(\tau/i)^n \sum_{\mu \in Q^\vee/Q}s_{0,\mu}\cdot \theta_{\mu+Q}(\tau)
$$
where the $s_{0,\mu} \in \mathbb{C}^\times$ are nonzero complex numbers
and $\tau_{\mu+Q}(\tau)=\sum_{\beta\in\mu+Q}q^{(\beta,\beta),2}$.
\end{proposition}

\noindent
As opposed to the usual case, this function has very
fractional powers of $q$. However, it is well-defined as
a function on $\mathbb{H}$.

\begin{proposition}
\label{proposition-}
If $Q$ is integral, then for all $\lambda \in Q^\vee/Q$,
$\theta_{\lambda+Q}\left(-\frac{1}{\tau}\right)
=(\tau/i)^n \sum_{\mu \in Q^\vee/Q}s_{\lambda\mu}\cdot\theta_{\mu+Q}(\tau)$.
\end{proposition}

\noindent
Again proved by Poisson summation.
In the process an explicit formula for $\{S_{\lambda\mu}\}$ emerges.
I can't remember, but it's something like $$
S_{\lambda\mu}=\frac{1}{|Q^\vee/Q|^{1/2}}e^{2\pi i(\lambda,\mu)}
\qquad (?)
$$
Thinking of $Q^\vee/Q$ as a finite list
$\{1,2,\ldots,N\}$, $N=|Q^\vee/Q|$, then
$\{S_{\lambda\mu}\}_{\lambda,\mu=1,\ldots,n}$ becomes a matrix,
the {\it $S$-matrix} of $Q$.

Since $-1\frac{1}{\left(-\frac{1}{\tau}\right)}=\tau$,
we (naively) should have $S^2=I$ identity matrix.

There is a slight problem: $\theta_{\mu+Q}(\tau)$
...
In fact, two of the theta-functions associated to these
...
$$
\theta_{\mu+Q}(\tau,z)=\sum_{\beta \in \mu+Q}q^{\frac{(\beta,\beta)}{2}}
e^{2\pi i(z,\beta)}.
$$
Now
$$
\theta_{\mu+Q}:\mathbb{H}\times\mathfrak{h}\to\mathbb{C}
$$
...
$$
\theta_{\lambda+Q}\left(-\frac{1}{\tau},\frac{z}{\tau}\right)
=(\tau/i)^ne^{\frac{\pi i(z,z)}{\tau}}
\sum_{\mu \in Q^\vee/Q}S_{\lambda,\mu}\theta_\mu(\tau,z)
$$
\end{proposition}

\begin{proof}
Also by Poisson summation, same $s_{\lambda,\mu}$.
But now $\{\theta_{\mu+Q}(\tau,z)|\mu \in Q^\vee/Q\}$ 
are linearly independent. So $S$ makes sense (as a matrix).
So it no longer holds that $S^2=I$.
We apply $S$-transformations as follows
\begin{align*}
(\tau,z)&\overset{s}{\mapsto }\left(-\frac{1}{\tau},\frac{z}{\tau}\right)
\mapsto \left(-\frac{1}{-\frac{1}{\tau}},\frac{z/\tau}{-1/\tau}\right)
=(\tau,-z)
\overset{s}{\mapsto }\left(-\frac{1}{\tau},-\frac{z}{\tau}\right)
...
Recall that
$$
\chi_{V_Q}(\tau)=\frac{1}{\prod_{m=1}^\infty(1-q^m)^n}\theta_Q(\tau).
$$

Recall the definition of the Dedekind $\eta$ function
(Theorem \ref{theorem-dedekind-eta-function}).
Why do we need to put the factor $q^{1/24}$?
The reason is that without it, it is not a modular form,
...
$$
$$
\chi_{V_Q}(\tau)=\frac{\theta_Q(\tau)}{\eta(\tau)^n}q^{n/24}.
$$

Let's correct the definition of $\chi_{V_Q}(\tau)$.
Note that $n=\text{rank}(Q)$ is the central charge $c$ of $V_Q$.

\begin{definition}
\label{definition-character-of-conformal-vertex-algebra}
The {\it character} of a conformal vertex algebra $V$ is
$$
\chi_V(\tau)=q^{-\frac{c}{24}}\sum_{n \in \mathbb{Z}}\dim(V_n)q^n.
$$
If $M$ is a $V$-module,
we also define $\chi_M(\tau)=q^{-\frac{c}{24}}\sum_{n\in!!}\dim(M_n)q^n$,
where $!!$ are the eigenvalues on $M$ of $L_0$ (maybe not in $\mathbb{Z}$).
...
For $\mu \in Q^\vee/Q$, we have the $V_Q$-module $V_\mu$.
Then
$$
\text{Tr}_{V_\mu}q^{L_0-c/24}=\frac{\theta_\mu(\tau)}{\eta(\tau)^n}
$$
and
$$
\boxed{\chi_M(\tau,z)=\text{Tr}_Me^{2\pi iz_0}q^{L_0-c/24}.}
$$
Indeed in $V_\lambda=\bigoplus_{\beta\in\lambda+Q}H^\beta(\mathfrak{h})$,
$z \in \mathfrak{h}$. In $H^\beta(\mathfrak{h})$,
$z_0|_{H^\beta}=(\beta,z)I_{H^\beta}$.
So $e^{2\pi i(z,\beta)}=e^{2\pi i z_0}$.
$$
\chi_{V_\lambda}\left(0\frac{1}{\tau},\frac{z}{\tau}\right)
=e^{\frac{\pi i(z,z)}{\tau}}
\sum_{\mu \in Q^\vee/Q}S_{\lambda,\mu}\chi_\mu(\tau,z).
$$
\end{lemma}
...
\medskip\noindent
(New lecture.) Let's do a summary of what we have done
in this section so far.
For $Q \subset \mathbb{R}^n$ an integral lattice,
we defined a function
$$
\theta_Q:\underbrace{\mathbb{H}}_{\{\text{Im}>0\}}x \mathfrak{h} \to \mathbb{C}
$$
by
$$
\theta_Q(\tau,z)=\sum_{\alpha \in Q}q^{\frac{(\alpha,\alpha)}{2}}
e^{2\pi i(\alpha,z)},
$$
where $q=e^{2\pi i \tau}$, $z \in \mathfrak{h}=\mathbb{C}^n$.
We explained that this series converges since $|q|<1$ and it
is raised to a quadratic term.

More generally, if
$[\mu] \in Q^\vee/Q$, then
$$
\theta_\mu(\tau,z)=\sum_{\alpha \in \mu+Q}q^{\frac{(\alpha,\alpha)}{2}}
e^{2\pi i (\alpha,z)}
$$
similarly.
...
If $Q$ is self-dual,
$$
\theta_Q\left(-\frac{1}{\tau},\frac{z}{\tau}\right)
...
$$
\theta_\lambda\left(-\frac{1}{\tau},\frac{z}{\tau}\right)
...
There is also the $T$-matrix, corresponding to
$(\tau,z)\mapsto (\tau+1,z)$.
\begin{align*}
S&\xymatrix{\ar@{<->}[r]&}
\begin{pmatrix}
...
\noindent
In fact, these generate $\text{SL}_2(\mathbb{Z})$.
\end{remark}

\begin{align*}
T&:\underbrace{e^{2\pi i \tau}}_{q}\to e^{2\pi i(\tau+1)}
=e^{2\pi i \tau}=q\\
...
\medskip\noindent
In $\theta_{\mu+Q}(\tau,z)$,
we have powers $q^{\frac{(\alpha,\alpha)}{2}}$ such that $\alpha \in \mu+Q$.
Note that if $\mu,\mu' \in \mu+Q$, $\mu'-\mu \in Q$,
$$
\frac{(\mu+\alpha,\mu+\alpha)}{2}
=\frac{(\mu,\mu)}{2}
+\underbrace{\frac{2(\mu,\alpha)}{2}}_{\in \mathbb{Z}}
+\underbrace{(\alpha,\alpha)}_{\in \mathbb{Z}}
$$
since $Q$ is even.

So $T$ acts on $\theta_\mu(\tau,z)$
by a constant. Indeed, by the constant
$e^{2\pi i (\mu,\mu)/2} \in \mathbb{C}^\times$.

So we have a $T$-matric, which is diagonal, with these numbers
$$
\{e^{2\pi i (\mu,\mu)/2}|\mu \in Q^\vee/Q\}
$$
on the diagonal.

In $\text{SL}_2(\mathbb{Z})$, $S^4=I$, $(ST)^3=I$.
These relations hold for our matrices $S$ and $T$.

\begin{proposition}
\label{proposition-lattices-and-sl2-representations}
An even integral lattice $Q$ 
gives a representation of $\text{SL}_2(\mathbb{Z})$ 
...
$$
is the graded dimension of the $V_Q$-module  $V_{\mu+Q}$.
More precisely,
$V=V_Q\mathbb{y}M=V_{\mu+Q}$.
$z \in \mathfrak{h}\subset H\subset V_Q$,
so $z_0$ acts on $V_{\mu+Q}$,
also $L_0$ acts on $V_{\mu+Q}.$ 
And
$$
\chi_{\mu+Q}(\tau,z)=\text{Tr}_{V_{\mu+Q}}e^{2\pi i z_0}
q^{L_0-c/24}
$$
where the central charge is $c=n=\text{rank}(Q)$.

\medskip\noindent
Recall (compare with Definition \ref{definition-modular-form}) that

\begin{definition}
\label{definition-modular-form2}
A {\it modular form} of weight $k$ is a holomorphic function
...
let $\Lambda=\{m+n\tau|m,n \in \mathbb{Z}\}\subset\mathbb{C}$
a ``lattice spanned by 1 and $\tau$''.
(It's not a lattice in the sense of Section \ref{section-lattices}.)
Consider
$$
...
$$
\begin{enumerate}
\item $G_m(\Lambda)$ can be thought of as $G_m(\tau)$.
\item The infinite series converges if $m \geq 3$.
Inside the disk $D_{2^N}$, there are
on the order of $(2^N)^2$ points.
These points have
$$
\left|\frac{1}{\lambda^m}\right|<\frac{1}{(2^{N-1})^m}.
$$
$$
...
\end{pmatrix}$,
$a,b,c,d$ integers,
and  $a\tau+b \in \Lambda$, $c\tau+d \in \Lambda$.
E.g. $\begin{pmatrix}
2&5\\ 
1&3
\end{pmatrix}$.
...
then
$$
\{m(a\tau+b)+n(c\tau+d)|m,n\in \mathbb{Z}\}=\Lambda.
$$
So, denoting $\sum'=\sum_{(m,n) \in \mathbb{Z}^2\setminus(0,0)}$,
\begin{align*}
G_k(\tau)&=\sum'\frac{1}{(m\tau+n)^k}\\
&=\sum'\frac{1}{(m(a\tau+b)+n(c\tau+d))^k}\\
&=(c\tau+d)^{-k}\sum'\frac{1}{\left(
m\frac{a\tau+b}{c\tau+d}+n\right)^k}\\
&=(c\tau+d)^{-k}G_k\left(\frac{a\tau+b}{c\tau+d}\right).
\end{align*}

\noindent
So $G_k\left(\frac{a\tau+b}{c\tau+d}\right)G_k(\tau)$ 
for all $\begin{pmatrix}
a&b\\ 
c&d
\end{pmatrix}\in \text{SL}_2(\mathbb{Z})$.
This is called an {\it Eiseistein series} of weight $k$.

\begin{remark}
\label{remark-firs-eisestein-series}
If $k$ odd, then $\frac{1}{\lambda^m}$ and $\frac{1}{(-\lambda)^m}$ 
cancel, and $G_k(\tau)=0$, nontrivial.
So $G_4(\tau)$ is the first Eisenstein series.
\end{remark}

\medskip\noindent
Now let's talk about the Fourier series of $G_{2k}(\tau)$.
Start with
\begin{equation}
\label{equation-euler}
(1-x^2)(1-\frac{x^2}{4})(1-\frac{x^2}{9}\ldots=\frac{\sin(\pi x)}{x},
\end{equation}

\noindent
which may be proved by applying Liouville's theorem
to the quotient LHS/RHS.

In fact, this was Euler's method for proving the identity
$\sum_{n=1}^\infty \frac{1}{n^2}=\frac{\pi^2}{6}$.

\medskip\noindent
Do log derivative of Equation \ref{equation-euler}.
$$
\pi \frac{\sin(\pi x)}{\cos(\pi x)}
=\frac{d}{dx}\text{log}\left(\prod_{n=1}^\infty
...
\label{theorem-g2k}
\begin{align*}
G_{2k}(\tau)&=(-1)^k\frac{\pi^{2k}}{(2k)!}B_{2k}\cdot E_{2k}(\tau),\\
E_{2k}(\tau)&=1-\frac{4k}{B_{2k}}\sum_{m=1}^\infty
...
B_{2k}:& 1 & 1/6 & 1/30 & 1/42 & -1/30 & 5/66 & -691/2730 & 7/6
\end{matrix}
$$
So
$$
E_4(\tau)=1-240 \sum_{m=1}^\infty \frac{m^3 q^m}{1-q^m},
$$
$$
...
\begin{definition}
\label{definition-space-of-holomorphic-modular-forms}
Let $\mathcal{M}^{\text{hol}}_{2k}$ be the vector space
...
& 0 & 0 & 1 & 1 & 1 & 1 & 2 & 1 & 2 & 2 & \leavevmode
\end{matrix}
$$
and $\dim\mathcal{M}^{\text{hol}}_{2k+12}=\dim\mathcal{M}^{\text{hol}}_{2k}+1$.
\end{proposition}

\begin{proof}[Proof sketch]
Let $f(\tau) \in \mathcal{M}^{\text{hol}_{2k}}$ 
and consider
$$
\xi(\tau)=\frac{d}{d\tau}\text{log}f(\tau)=\frac{f'(\tau)}{f(\tau)}.
$$
Since
\begin{align*}
f(-1/\tau)&=\tau^{2k}f(\tau),\\
...
\noindent
apply Cauchy-formula-type argument $\int_{\partial \mathcal{D}}\xi(\tau)d\tau
=$ numer of zeroes of $f -$ number of poles of  $f$ in $\mathcal{D}$.

Then integrate along the boundary of the region above the unit circle,
between the vertical lines $x=-1/2,1/2$ and bounded above by
some horizontal segment at height $N$.
In conclusion, the vertical integrals cancel each other
and $\frac{k}{12}\geq N(i\infty)$,
where $N(i\infty)=$ order of the zero of $f$ at $i\infty$.
\end{proof}

\noindent
One has, if $f \in \mathcal{M}_{2k}^{\text{hol}}$,
$g \in \mathcal{M}_{2\ell}^{\text{hol}}$,
$fg \in \mathcal{M}^{\text{hol}}_{2(k+\ell)}$.
So $E^3_4 \in \mathcal{M}^{\text{hol}}_{12}$ and
$E_{12}\in \mathcal{M}_{12}^{\text{hol}}$.

Then do
\begin{align*}
E_{12}-E^3_4&=\frac{65520}{691}(q-24q+252q-\ldots)\\
&=\frac{65520}{691}q\prod_{n=1}^\infty(1-q^N)^{24}.
\end{align*}
...
\noindent
In fact, $\bigoplus_{k=0}^\infty\mathcal{M}_{2k}^{\text{hol}}
...
So
$$
P(\tau,z)=\frac{\theta_Q(\tau,z)}{\eta(\tau)^{24}}
$$
satisfies
$$
...
E_2(\tau)&=1-24\sum_{n=0}^\infty\frac{n q^n}{1-q^n}.
\end{align*}

\noindent
The series $\sum_{\lambda \in \Lambda\setminus 0}\frac{1}{\lambda^2}$ 
does not converge absolutely, but converges conditionally.
...
$$
[\text{Picture of diagonal dor arrangement in plane}]
$$
...
\substack{\sum_{\lambda \in \Lambda \setminus 0}\frac{1}{\lambda^2}, \\
 \text{but summed in }\\
\text{different order}}}].
$$
\begin{proposition}
...
$$
E_2(-1/\tau)=\tau^2E_2(\tau)+\frac{12\tau}{2\pi i}.
$$
\end{proposition}
...
Idea: consider $\frac{\eta'(\tau)}{\eta(\tau)}$ 
and relate it to $E_2(\tau)$.
\end{remark}

\noindent
$$
\bigoplus_{k=0}^\infty\mathcal{M}^{\text{hol}}_{2k}
\cong\mathbb{C}[G_4,G_6].
$$

\medskip\noindent
Recall we were working with an even self-dual lattice $Q$ 
of rank (say) 24. [This case is actually simpler thank
ranks 8 or 16 since we don't get fractional powers of $q$.]
We had
$$
\theta_Q(\tau,z)=\sum_{\alpha \in Q}q^{\frac{(\alpha,\alpha)}{2}}
e^{2\pi i(\alpha,z)}.
...
$$
\theta_Q\left(-\frac{1}{\tau},\frac{z}{\tau}\right)
=e^{\frac{\pi i (z,z)}{\tau}}(\tau/i)^{n/2}\theta_Q(\tau,z)
$$
Let's consider
$$
P_Q(\tau,z)=e^{\frac{-2\pi i(z,z)}{24}E_2(\tau)}
\theta_Q(\tau,z).
$$

We can check that the non-modularity of $E_2(\tau)$ 
``cancels'' the $e^{\frac{\pi i (z,z)}{\tau}}$ factos,
and we get
$$
P_Q\left(-\frac{1}{\tau},\frac{z}{\tau}\right)=(\tau/i)^{n/2}P_Q(\tau,z).
$$

\begin{proof}
\begin{align*}
e^{-2\pi i \frac{\left(\frac{z}{\tau},\frac{z}{\tau}\right)}{24}
E_2\left(-\frac{1}{\tau}\right)}
&=e^{2\pi i \frac{(z,z)}{24\tau^2}
\left(\tau^2E_2(\tau)+\frac{12\tau}{2\pi i}\right)}\\
&=e^{-2\pi i \frac{(z,z)}{24}E_2(\tau)-\frac{(z,z)}{2\tau}},
\end{align*}
...
\medskip\noindent
Let's Taylor series expand $P_Q$ (in $\mathbb{Z}$).
\begin{align*}
\theta_Q(\tau,z)
&=\sum_{\alpha \in Q}q^{\frac{(\alpha,\alpha)}{2}}
\left(1+2\pi i(\alpha,z)
+\frac{(2\pi i)^2}{2}(\alpha,z)^2
+\frac{(2\pi i)^2}{6}(\alpha,z)^3+\ldots\right)\\
&=\sum_{\alpha \in Q}q^{\frac{(\alpha,\alpha)}{2}}
+2\pi i\underbrace{\sum_{\alpha\in Q}q^{\frac{(\alpha,\alpha)}{2}}(\alpha,z)
}_{\substack{=0\text{ since} \\ \alpha,-\alpha\text{ cancel }\forall \alpha \in
Q}}
+\frac{(2\pi i)^2}{2}\sum_{\alpha \in Q}q^{\frac{(\alpha,\alpha)}{2}}
(\alpha,z)^2+\ldots
\end{align*}
...
+\frac{(2\pi i)^2}{24^2}(z,z)^2E_2(\tau)^2+\ldots
$$
Multiply to get
$$
P_Q(\tau,z)=\theta_Q(\tau)+
\left(\theta_Q(\tau)\frac{-2\pi i}{24}(z,z)E_2(\tau)
+\frac{(2\pi i)^2}{2}
\sum_{\alpha \in Q}q^{\frac{(\alpha,\alpha)}{2}}(\alpha,z)^2\right)+\ldots
$$
Expanding further the $z^2$ term:
\begin{align*}
-\frac{2\pi i}{24}\underbrace{(1-24q+\ldots)
}_{E_2(\tau)}
\sum_{\alpha \in Q}q^{\frac{(\alpha,\alpha)}{2}}(z,z)
+\frac{(2\pi i)^2}{2}\sum q^{\frac{(\alpha,\alpha)}{2}}(\alpha,z)^2
\end{align*}
...
&+\frac{1}{2}\left(
\sum_{(\alpha,\alpha)=2}q(\alpha,z)^2+O(q^2)\right)\\
&=-\frac{(z,z)}{24}
+q\left(\frac{1}{2}\sum_{(\alpha,\alpha)=2}(\alpha,z)^2
...
\medskip\noindent
What good is this?
Since $P_Q\left(-\frac{1}{\tau},\frac{z}{\tau}\right)
=\tau^{12}P_Q(\tau,z)$ (for $n=24$).
We have ($S$ and $T$-invariance, which generate
the whole modular group)
\begin{align*}
P_Q\left(\frac{a\tau+b}{c\tau+d},\frac{z}{c\tau+d}\right)
&=(c\tau+d)^{12}P_Q(\tau,z).\\
P_Q(\tau,z)&=
...
which is 1-dimensional, spanned by $E_{14}(\tau)=1-24q+\ldots$.

For us, this means 
that $\text{coef}(q)=-24\text{coef}(1)$ in the last equality of
Equation \ref{equation-e14}.
I.e.,
\begin{equation}
\label{equation-coefficients}
\frac{1}{2}\sum_{(\alpha,\alpha)=2}(\alpha,z)^2
-\frac{(z,z)}{24}(N-24)=(z,z).
\end{equation}
$$
\frac{1}{2}\sum_{(\alpha,\alpha)=2}(\alpha,z)^2=(z,z)\frac{N}{24}.
$$
Recall that
$$
...
$\Delta$ is either $A_n$, $D_n$, $E_6$, $E_7$ or $E_8$
(or an orthogonal sum of such components).
A simple root system $\Delta$ has 
an associated invariant called the dual Coxeter number
...
The following lemma claims that this form is unique.

\begin{lemma}
\label{lemma-uniqueness-of-bilinear-form}
If $\left<\cdot,\cdot\right>:\mathbb{R}^n\times\mathbb{R}^n\to\mathbb{R}$
...
$$
This is $W$-invaraint, and so is some multiple of $(\cdot,\cdot)$.
...
In particular,
$$
\frac{1}{2}\sum_{\alpha \in \Delta}(\alpha,z)^2=h^\vee(z,z),
$$
which appears in Equation \ref{equation-coefficients}.
...
=\Delta_1\cup \Delta_2\cup \ldots\cup \Delta_k
$$
where $\Delta_i$ is a simple root system.

Let $\beta\in\Delta_i$, our formula implies
\begin{align*}
h_i^\vee(\beta,\beta)&=(\beta,\beta)\frac{N}{24}\\
...
If $\Delta=\Delta_i$ is simple and type $A_n$,
say $A_{24}$. We have $h^\vee=25$,
\begin{align*}
\dim \mathfrak{sl}_{25}&=25^2-1=(25+1)(25-1)\\
...
\item $E_8^3$: $|\Delta_{E_8}|=240$, $h^\vee=30$,
$h^\vee=\frac{3\times 240}{24}=30$, is a solution.
\item $A_2^{24}$, $h^\vee=2$ for all components,
$\frac{|\Delta|}{24}=\frac{24(2)}{24}=2$.
\end{itemize}
\end{remark}

\noindent
Consider $\mathfrak{g}$ a semisimple Lie algebra with root system $\Delta$,
$\mathfrak{g}=\mathfrak{g}_1\oplus\mathfrak{g}_2 \oplus \ldots 
\oplus \mathfrak{g}_k$,
$\Delta(\mathfrak{g}_i)=\Delta_i$.

Note that if $\Delta\neq \emptyset$, then $\text{rank}(\Delta)=24$.
So we can rewrite $\#\Delta$ as $\dim \mathfrak{g}-24$.
so
\begin{equation}
\label{equation-hi}
...
\noindent
There exist at most ``partitions of 24 into 3 colours
A, D and E'' solutions to Equation \ref{equation-hi}.

Observe: $\ell:$ rank, $d:$ dimension, $h^\vee:$ $h^\vee$.
For ADE, $h^\vee=O(\ell)$, $d=O(\ell^2)$.
Use this to bound $\dim(\mathfrak{g})$.

Turns out there are 23 solutions (24 if we include the 
empty root system)
$A_{24},E_8^3,A_1^{24},A_2^{12}$, etc.

\begin{theorem}
\label{theorem-solutions}
Each nonempty solution $\Delta$ is the root system
of exactly one self-dual even lattice.
\end{theorem}

\begin{proof}
Construct $Q_\Delta$ by orbifolding from $E_8^3$.
Compute $|\text{Aut}(Q_\Delta)|$,
 $$
RHS(\text{mass formula})-\sum_{\Delta \neq \emptyset}
\frac{1}{|\text{Aut}(Q_\Delta)|}=\frac{1}{10^{18}}.
$$
\end{proof}

\medskip\noindent
The subalgebra $\mathfrak{g}_i\subset \mathfrak{g}$
...
namely $V_Q$.
Let $V=V_0 \oplus V_1 \oplus V_2 \oplus \ldots$
...
\noindent
It turns out that $\mathfrak{g}=\mathfrak{g}_1 \oplus \mathfrak{g}_2
\oplus \ldots \oplus \mathfrak{g}_k$ 
induces a set of embeddings 
$$
L_{k_i}(\mathfrak{g}_i) \to V,\qquad  k_i \in \mathbb{Z}_{\geq 1}.
$$
One can prove that
$$
\frac{h_i^\vee}{k_i}
