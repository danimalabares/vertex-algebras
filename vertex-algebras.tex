...
$$
\chi_{L(\Lambda_0)}=\left(\prod_{k=1}^\infty
\frac{1}{1-q^k}\cdot \sum_{m \in \mathbb{Z}}y^mq^{m^2}\right)
$$
...
\begin{align*}
\prod_{n=1}^\infty&=(1-yq^{n-1})(1-q^n)(1-y^{-1}q^n)\\
&=\sum(\text{exercise})
\end{align*}
...
\medskip\noindent
{\bf Returning to $\hat{\mathfrak{a}}\mathbb{y}H=H^\circ$.} Introduce
$$
...
\begin{align*}
[h(z),h(z)]&=\sum_{m,n \in \mathbb{Z}}&=\sum_{m,n}z^{-(m+n)-2}\\
&=\sum z^{-p-2}\left(\underbrace{\sum_{m+n=2}p[h_m,h_n}_{\text{infinite?}}
\right)
\end{align*}
...
$$
\end{definition}
Why (Dirac) delta function? Recall that $\delta(x)$ is the 
...
\begin{equation}
\label{equation-Bocherds-identity}
\begin{aligned}
&\sum_{j \geq 0}\binom{m}{j}(a_{(n+j)}b)_{(m+k-j)}c\\
&=\sum_{j \geq 0}(-1)^j \binom{n}{j}
\left(a_{(m+n-j)}b_{(k+j)}c-(-1)^nb_{(n+k-j)}a_{(n+j)}c\right)
\end{aligned}
\end{equation}

This is called {\it Bocherd's identity}, and is the key
...
\item Equation \ref{equation-Bocherds-identity} holds
...
[This going too fast so I take photo] But the first part shows that
$x_1$ corresponds to the sort of ``generating field'' $h(z)$. What about $x_2$?

We conclude that
$$
Y(x_{n+1},z)=\partial_z^{(n)}h(z).
$$
The next question is: what about $x^2$? It turns out that
\begin{align*}
...
Bocherd's formula \ref{equation-Bocherds}.

First let's comput $(h_{(-1)}h)_{(0)}L$, i.e. we
...
[a_\lambda \,:\!bc\!:\,]=\,:\![a_\lambda bc\!:\,+\,:\!b[a_\lambda c\!:\,
...
given by
$$
\rho(\underbrace{L_n}_{\substack{\text{as an abstract} \\ \text{object in Vir}}}
