...
=\left\{(a_{ij}):\substack{rr N s.t. a_{ij}=0 \\ \text{ whenever }
|i-j|>N}\right\}.
$$
...
Consider the vector speca $\mathbb{C}(\!(t)\!)=X$.
...
Notice that
\begin{align*}
\text{End}_c(X) \oplus \text{End}_d(X)  &\longrightarrow \text{End}(X)\to  0\\
(f,g) &\longmapsto f+g
\end{align*}
...
\end{pmatrix}.
$$
We can complete this to a sort exact sequence by putting
\begin{align*}
\text{End}_f(X)  &\longrightarrow \text{End}_c \oplus \text{End}_d \\
h &\longmapsto (h,-h).
\end{align*}
...
$$
In fact, this completes
to a short exact sequence of Lie algebras
(Exercise \ref{exercise-short-exact-sequence} below):
$$
\xymatrix{
0\ar[r]&\text{End}_f\ar[r]\ar[d]_{\text{Tr}}&
\text{End}_c(X) \oplus \text{End}_d(X)\ar[d]\ar[r]&
...
\begin{exercise}
\label{exercise-short-exact-sequence}
Show that
$$
\xymatrix{
0\ar[r]&\mathbb{C}\ar[r]&L\ar[r]&\text{End}(X)\ar[r]&0
...
i.e. a {\bf central extension} of $\text{End}(X)$.
(Don't forget these are {\it continuous} endomorphisms.)
\end{exercise}

\noindent
Let $\mathfrak{gl}(X)=\text{End}(X)$ 
with Lie bracket $[A,B]=AB-BA$.
[Beilinson-Drinfeld] denote $L=\mathfrak{gl}^\flat$ in this case.

\begin{example}[Computation]
\label{example-computation}
Let $A = \Lambda_1:t^k \mapsto  t^{k+1}$ for all $k \in \mathbb{Z}$
and $B=\Lambda_{-1}:t^k \mapsto  t^{k-1}$ for all $k \in \mathbb{Z}$.
Then $A,B \in \text{End}(X)$ and $[A,B]=0$ in fact.
Let's compute $[A,B]$ in $\mathfrak{gl}^\flat(X)$,
we hope it's not zero.

The idea is to split both $A$ and $B$ 
as sums of a matrix in $\text{End}_c$ and
a matrix in $\text{End}_d$.
Then we compute the brackets
in $\text{End}_c$ and in $\text{End}_d$
and obtain that
$[A_c,B_c]=-[A_d,B_d]$, nice.
That is, in $\mathfrak{gl}^\flat(X)$,
$$
[A,B]=(-E_{-1,-1},E_{-1,-1},0)=(0,0,-1)
$$
by our convention that $(h,-h,0)=\text{Tr}(h))$.
\end{example}

\begin{remark}
\label{remark-tate-observed}
If $fg \in \mathbb{C}(\!(t)\!)$,
Tate observed that
$$
[\mu(f),\mu(g)]=\text{Res}_tf(t)dg(t).
$$
